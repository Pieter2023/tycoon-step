    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white pb-24 md:pb-4">
      {showTown && <TabErrorBoundary tabName="3D neighbourhood"><React.Suspense fallback={<Modal isOpen onClose={() => setShowTown(false)} ariaLabel="Loading neighbourhood"><p className="p-8">Opening Freedom Square…</p></Modal>}>
        <TownModal state={gameState} disabled={isProcessing || !!gameState.pendingScenario || gameState.hasWon || gameState.isBankrupt} reduceMotion={!!reduceMotion}
          ...
          workActions={monthlyActionsSummary} onMonthlyAction={handleUseMonthlyAction} onEnroll={handleEnrollEducation} onBuyVehicle={(id,finance)=>{if(isProcessing)return;const next=buyVehicle(gameState,id,finance);if(next===gameState){showNotif('Not now','That car is not available on those terms.','warning');return;}setGameState(next);recordAutosave(next);playPurchase();showNotif('Keys in hand',finance?'The loan payment is in your bills from next month.':'Paid in cash. Upkeep starts next month.','success');}} onSellVehicle={id=>{if(isProcessing)return;const next=sellVehicle(gameState,id);if(next===gameState){showNotif('Not now','The dealer cannot take that car right now.','warning');return;}setGameState(next);recordAutosave(next);showNotif('Sold','The dealer\'s money is in your cash.','success');}} onClaimQuest={handleClaimQuest} onOpenQuests={()=>{setShowTown(false);setShowQuestLog(true);}} onBuyPolicy={(id,deductible)=>{if(isProcessing)return;const next=buyPolicy(gameState,id,deductible);if(next===gameState){showNotif('Not now','That policy is not available right now.','warning');return;}setGameState(next);recordAutosave(next);showNotif('Covered','Premiums start next month.','success');}} onCancelPolicy={id=>{if(isProcessing)return;const next=cancelPolicy(gameState,id);if(next===gameState)return;setGameState(next);recordAutosave(next);showNotif('Cover cancelled','No premium from next month.','warning');}} onCollegeFund={(childId,amount)=>{if(isProcessing)return;const next=contributeCollegeFund(gameState,childId,amount);if(next===gameState){showNotif('Not now','That transfer could not be made.','warning');return;}setGameState(next);recordAutosave(next);showNotif('College fund',`$${amount.toLocaleString('en-US')} moved to savings for college.`,'success');}}
