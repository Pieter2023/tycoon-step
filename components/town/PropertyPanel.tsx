    ...
    <div className="town-offers">
      {PROPERTY_LISTINGS.map(id => {
        ...
        return <article key={id} className="town-offer">
          ...
          <p>{tl('Gross rent about','Renta bruta de unos')} <strong>{money(month.grossRent)}/{tl('mo','mes')}</strong> ({(incomeYield(item) * 100).toFixed(1)}%/{tl('yr','año')}). {tl('Less','Menos')} {money(month.upkeep)} {tl('upkeep, property tax and insurance, and','de mantenimiento, impuesto predial y seguro, y')} {money(month.vacancy)} {tl('vacancy allowance leaves','de reserva por vacancia deja')} <strong>{money(month.net)}/{tl('mo','mes')}</strong> {tl('before any loan or tax.','antes de préstamo o impuestos.')}</p>
          {item.canMortgage ? <>
