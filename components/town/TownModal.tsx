...
export default function TownModal({ state, disabled, reduceMotion, onBuy, onSell, onMortgage, onChangeLifestyle, onPromote, onOpenLife, onAskRaise, onSwitchCareer, onJobSearch, onAcceptPlan, onStartHustle, onStopHustle, onChooseUpgrade, onFileClaim, workActions, onMonthlyAction, onEnroll, onCollegeFund, onBuyPolicy, onCancelPolicy, onClaimQuest, onOpenQuests, onBuyVehicle, onSellVehicle, onClose, onOpenMoney, onNextMonth, saveError, onBackup, onAction, onRememberView, onTransfer, onRunShift, loans=[], onFinishJourney, onCafeAction, onCafeServiceAction, onMilestones, onSleep }: Props) {
  ...
  return <Modal isOpen onClose={onClose} ariaLabel="Freedom Square 3D neighbourhood" showCloseButton={false} overlayStyle={{ padding: 0 }} contentStyle={{ maxWidth: 1500, width: '100%' }} contentClassName={`town-modal${focusView?' town-focused':''}${serviceActive&&room==='cafe'?' town-in-service':''}`}>
    ...
    {room==='college'&&<div className="town-interior-nav"><span>{tl('Community College','Colegio Comunitario')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToRegistrar?.();}}>{tl('Walk to the registrar','Ir con la secretaria')}</button><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToExit?.();}}>{tl('Walk to exit','Ir a la salida')}</button><button onClick={()=>{cancelGuide();controller.current?.leaveCollege?.();}}>{tl('Back to the square','Volver a la plaza')}</button></div>}
    ...
    {room!=='city'&&<button className="town-location-help" onClick={()=>{setRoomBrief(true);setJournal(false);setShowDetails(true);}}>{tl('What can I do here?','¿Qué puedo hacer aquí?')} · {tl('Actions, costs & rewards','Acciones, costos y recompensas')}</button>}
    <div className="town-journey-strip">
      <button className="town-journey-summary" disabled={serving} onClick={()=>{setRoomBrief(false);setJournal(true);setShowDetails(true);}}><span>{journey.completed&&journey.stage===3?tl('THIS MONTH','ESTE MES')+' · '+challengesDone+'/3 '+tl('CHALLENGES','RETOS'):journey.completed?tl('BADGES EARNED','INSIGNIAS GANADAS'):journey.stage===3?tl('NEIGHBOURHOOD TOUR','RECORRIDO DEL BARRIO')+' · '+(journey.step+1)+'/5':journey.stage===2?tl('INVESTOR JOURNEY','RECORRIDO DEL INVERSOR')+' · '+(journey.step+1)+'/4':tl('YOUR FIRST BUSINESS','TU PRIMER NEGOCIO')+' · '+(journey.step+1)+'/5'}</span><strong>{journey.completed&&journey.stage===3?monthlyChallenges(state).map(c=>c.title).join(' · '):journey.title}</strong><small>{journey.completed?tl('Journey & monthly recap →','Recorrido y resumen mensual →'):tl('Steps & monthly recap →','Pasos y resumen mensual →')}</small></button>
