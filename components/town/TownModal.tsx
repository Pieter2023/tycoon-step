import CafeCinematic from './CafeCinematic';
import CafeServicePanel from './CafeServicePanel';
import TownLocationGuide, { type GuideLocation } from './TownLocationGuide';
...
import { noticeSheet } from '../../services/townQuests';
import { garageFigures } from '../../services/townGarage';
...
type Props = {
  ...
  onBuyPolicy?: (id: PolicyId, deductible: number) => void;
  onClaimQuest?: (id: string) => void;
  onBuyVehicle?: (listingId: string, finance: boolean) => void;
  onSellVehicle?: (vehicleId: string) => void;
  onOpenQuests?: () => void;
  onCancelPolicy?: (id: PolicyId) => void;
  ...
  onBackup: () => void;
  /** Milestones the city just celebrated, to record in the save so each is celebrated once. */
...
export default function TownModal({ state, disabled, reduceMotion, onBuy, onSell, onMortgage, onChangeLifestyle, onPromote, onOpenLife, onAskRaise, onSwitchCareer, onJobSearch, onAcceptPlan, onStartHustle, onStopHustle, onChooseUpgrade, onFileClaim, workActions, onMonthlyAction, onEnroll, onCollegeFund, onBuyPolicy, onCancelPolicy, onClaimQuest, onOpenQuests, onBuyVehicle, onSellVehicle, onClose, onOpenMoney, onNextMonth, saveError, onBackup, onAction, onRememberView, onTransfer, onRunShift, loans=[], onFinishJourney, onCafeAction, onCafeServiceAction, onMilestones, onSleep }: Props) {
  ...
  // Phone check 2026-09-26: on a portrait phone only three of the twelve destinations fit, with no sign that the row
  // scrolls. Edge buttons appear whenever the row overflows, and the chosen destination scrolls into view.
  const destinationsRef = useRef<HTMLElement>(null);
  const [destinationScroll, setDestinationScroll] = useState({ left: false, right: false });
  const updateDestinationScroll = useCallback(() => {
    const nav = destinationsRef.current; if (!nav) return;
    const left = nav.scrollLeft > 4, right = nav.scrollLeft + nav.clientWidth < nav.scrollWidth - 4;
    setDestinationScroll(current => current.left === left && current.right === right ? current : { left, right });
  }, []);
  const scrollDestinations = (direction: number) => { const nav = destinationsRef.current; nav?.scrollBy?.({ left: direction * nav.clientWidth * .7, behavior: reduceMotion ? 'auto' : 'smooth' }); };
  ...
  // Opt-in performance readout for device checks: open the game with ?stats in the URL.
  const [statsOn]=useState(()=>typeof window!=='undefined'&&new URLSearchParams(window.location.search).has('stats'));
  const [stats,setStats]=useState<{fps:number;calls:number;triangles:number}|null>(null);
  ...
  const [celebrating,setCelebrating]=useState<{title:string;text:string}|null>(null);
  // Going to bed: a short fade through the night to the next month's morning.
  const [sleeping,setSleeping]=useState<number|null>(null);
  useEffect(() => {
    const nav = destinationsRef.current;
    if (nav && typeof ResizeObserver !== 'undefined') { const observer = new ResizeObserver(updateDestinationScroll); observer.observe(nav); return () => observer.disconnect(); }
    window.addEventListener('resize', updateDestinationScroll); return () => window.removeEventListener('resize', updateDestinationScroll);
  }, [updateDestinationScroll]);
  useEffect(() => { updateDestinationScroll(); });   // pills come and go (cart, garage, rooms), so re-measure after each render
  useEffect(() => { destinationsRef.current?.querySelector<HTMLElement>('[aria-pressed="true"]')?.scrollIntoView?.({ inline: 'nearest', block: 'nearest' }); }, [destination]);
  useEffect(()=>{if(sleeping===null)return;const timer=setTimeout(()=>setSleeping(null),reduceMotion?900:2600);return ()=>clearTimeout(timer);},[sleeping,reduceMotion]);
  ...
  const [roomBrief,setRoomBrief]=useState(false);
  const [room,setRoom]=useState<'city'|'bank'|'cafe'|'exchange'|'property'|'home'|'work'|'college'>('city');
  const [rosa,setRosa]=useState(false);
  const [timeOfDay,setTimeOfDay]=useState<string>('');
  const [board,setBoard]=useState(false);
  const [garageOpen,setGarageOpen]=useState(false);
  const openGarage=()=>{setJournal(false);setBoard(false);setRosa(false);setGarageOpen(true);setShowDetails(true);};
  const openBoard=()=>{setJournal(false);setRosa(false);setGarageOpen(false);setBoard(true);setShowDetails(true);};
  const openRosa=()=>{setJournal(false);setBoard(false);setGarageOpen(false);setRosa(true);setShowDetails(true);};
  const goTo=(place:NonNullable<Advice['place']>)=>{setShowDetails(false);setRosa(false);setBoard(false);setGarageOpen(false);if(place==='garage'){if(room==='city'&&spot==='garage'){openGarage();return;}cancelGuide();controller.current?.walkToGarage?.();return;}if(place==='board'){openBoard();return;}if(place==='home'){if(room==='home'){setShowDetails(true);return;}cancelGuide();controller.current?.walkHome?.();return;}if(place==='cafe'){visitCafe();return;}if(place==='work'){if(room==='work'){cancelGuide();controller.current?.walkToManager?.();return;}cancelGuide();controller.current?.walkToWork?.();return;}if(place==='college'){if(room==='college'){cancelGuide();controller.current?.walkToRegistrar?.();return;}guide('registrar',()=>controller.current?.walkToCollege?.());return;}move(place);};
  ...
  // After the guided journeys, the strip follows the one Freedom Track (services/freedomTrack.ts): its chapter and next milestone.
  const track=React.useMemo(()=>freedomTrack(state),[state]);
  const trackNext=track.current?.milestones.find(m=>m.status==='ready'||m.status==='active');
  ...
  const inspect = () => {
    ...
    if(room==='city'&&spot==='garage'){openGarage();return;}
    if(room==='city'&&spot==='work'&&!unavailable){controller.current?.enterWork?.();return;}
    if(room==='city'&&spot==='college'&&!unavailable){controller.current?.enterCollege?.();return;}
    if(room==='college'&&spot==='exit'){controller.current?.leaveCollege?.();return;}
  ...
  const releaseStick = (event?:React.PointerEvent<HTMLDivElement>|React.FocusEvent<HTMLDivElement>) => { if(event&&'pointerId' in event&&event.pointerId!==stickPointer.current)return; stickPointer.current = null; setStick({x:0,z:0}); controller.current?.move(0,0); };
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Alex plays as the AI-modelled hero on the townspeople's rig; every other character is a styled townsperson.
  ...
  useEffect(()=>{
    ...
    else if(hop==='enterProperty')controller.current?.enterProperty?.();
    else if(hop==='walkToAgent')controller.current?.walkToAgent?.();
    ...
    else if(hop==='enterCollege')controller.current?.enterCollege?.();
    else if(hop==='walkToRegistrar')controller.current?.walkToRegistrar?.();
    else if(hop==='arrived'){guided.current=null;setRoomBrief(false);setJournal(false);if(target==='rosa'){openRosa();return;}if(target==='board'){openBoard();return;}if(target==='cafe')setCafePlay(!state.cafe);setShowDetails(true);}
  ...
  useEffect(()=>{controller.current?.setSyllabus?.(collegeBoard(state));},[state,reduceMotion]);
  ...
  useEffect(()=>{
    ...
    const earned=completed!==undefined&&completedBefore.current!==completed?{title:'Neighbourhood entrepreneur',text:tl('You opened a business and learned what it earned.','Abriste un negocio y aprendiste lo que ganó.')}:investor!==undefined&&investorBefore.current!==investor?{title:'Patient investor',text:tl('You owned a slice of the market and held it through the noise.','Tuviste una parte del mercado y la conservaste a pesar del ruido.')}:null;
  ...
  useEffect(()=>{controller.current?.stageEvent?.(state.pendingScenario?eventPlace(state.pendingScenario.category,state.pendingScenario.id).place:null);},[state.pendingScenario,loading]);
  ...
  useEffect(()=>{
    ...
    const fresh=newMilestones(state);if(!fresh)return;
    const record=fresh.record.filter(id=>!celebratedMilestones.current.has(id));if(!record.length)return;
    record.forEach(id=>celebratedMilestones.current.add(id));onMilestones?.(record);
    setShowDetails(false);setCelebrating({title:fresh.celebrate.title,text:fresh.celebrate.text});controller.current?.moment?.(fresh.celebrate.moment);controller.current?.celebrate?.();
  },[state.assets,state.cafe,state.month,loading,disabled]);
  // Every badge clears itself, whatever raised it (a later state change must not leave one stuck on screen).
  useEffect(()=>{if(!celebrating)return;const timer=setTimeout(()=>setCelebrating(null),5000);return ()=>clearTimeout(timer);},[celebrating]);
  ...
  useEffect(()=>{controller.current?.setNotices?.(noticeSheet(state,t));},[state,loading,t]);
  useEffect(()=>{controller.current?.setGarage?.(garageFigures(state));},[state.vehicles,loading]);
  ...
  useEffect(()=>{if(room==='college'&&spot==='registrar'&&showDetails&&state.townProgress?.collegeVisitedMonth===undefined)onAction?.('visit-college');},[room,spot,showDetails]);
  ...
  const visitCafe=()=>{cancelGuide();setJournal(false);if(room==='cafe')setCafePlay(false);if(unavailable){setRoom('cafe');setShowDetails(true);}else if(room==='cafe'){setRoomBrief(false);setShowDetails(true);}else if(room==='city'&&near==='business'&&spot!=='cart'){controller.current?.enterCafe?.();}else move('business');};
  ...
  const followJourney=()=>{
    ...
    else {setRoomBrief(false);setJournal(true);setShowDetails(true);}
  };
  ...
  const roomTitle=room==='bank'?tl('Community Bank','Banco Comunitario'):room==='cafe'?tl('Little Square Café','Café de la Plazuela'):room==='exchange'?tl('The Exchange','La Bolsa'):room==='property'?tl('Property & Co.','Propiedades & Cía.'):room==='home'?tl('Your place','Tu casa'):room==='work'?tl('Main Street Offices','Oficinas de Main Street'):room==='college'?tl('Community College','Colegio Comunitario'):tl('Freedom Square','Plaza de la Libertad');
  const location:GuideLocation|undefined=room!=='city'?room:garageOpen?'garage':board?'board':rosa?'rosa':spot==='cart'?'cart':near??undefined;
  const continueRoom=()=>{
    setRoomBrief(false);setShowDetails(false);
    const c=controller.current;
    const target=room==='bank'?'teller':room==='exchange'?'broker':room==='property'?'agent':room==='home'?'desk':room==='work'?'manager':room==='college'?'registrar':null;
    if(target&&spot===target){setShowDetails(true);return;}
    if(room==='bank')guide('teller',()=>c?.walkToTeller());
    else if(room==='exchange')guide('broker',()=>c?.walkToBroker());
    else if(room==='property')guide('agent',()=>c?.walkToAgent());
    else if(room==='home')guide('desk',()=>c?.walkToDesk());
    else if(room==='work')guide('manager',()=>c?.walkToManager());
    else if(room==='college')guide('registrar',()=>c?.walkToRegistrar());
    else {setCafePlay(true);setShowDetails(true);}
  };
  const roomAction=room==='bank'?tl('Talk to the teller','Hablar con el cajero'):room==='exchange'?tl('Talk to the broker','Hablar con el corredor'):room==='property'?tl('Talk to the agent','Hablar con el agente'):room==='home'?tl('Use your desk','Usar tu escritorio'):room==='work'?tl('Talk to your manager','Hablar con tu jefe'):room==='college'?tl('Talk to the registrar','Hablar con la secretaria'):tl('Choose a café shift','Elegir un turno de café');
  const chooseCamera=(mode:CameraPreset)=>{setCameraMode(mode);controller.current?.setCamera?.(mode);setCameraTools(false);};
  return <Modal isOpen onClose={onClose} ariaLabel="Freedom Square 3D neighbourhood" showCloseButton={false} overlayStyle={{ padding: 0, overflow: 'hidden' }} contentStyle={{ maxWidth: 1500, width: '100%' }} contentClassName={`town-modal${focusView?' town-focused':''}${serviceActive&&room==='cafe'?' town-in-service':''}${room!=='city'?' town-in-room':''}`}>
    ...
    <div className="town-destinations-wrap">
    <nav ref={destinationsRef} onScroll={updateDestinationScroll} className="town-destinations" aria-label={tl(tl('Walk to a destination','Caminar a un destino'),'Caminar a un destino')}>
      ...
      {!unavailable&&<button aria-label={tl('Walk to your parking bay','Ir a tu cochera')} disabled={serving} onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToGarage?.();}} style={{'--place-color':'#c9a14d'} as React.CSSProperties}><span className="town-dot"/>{tl('Garage','Cochera')}{state.vehicles?.length?` 🚗`:''}</button>}
      {!unavailable&&<button aria-label={tl('Walk to the community college','Ir al colegio comunitario')} disabled={serving} onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToCollege?.();}} style={{'--place-color':'#b79ad6'} as React.CSSProperties}><span className="town-dot"/>{tl('College','Colegio')}{state.education?.currentlyEnrolled?.educationId?' 📚':''}</button>}
      ...
      <button aria-label={tl('Notice board: this month\'s challenges','Tablón de anuncios: los retos de este mes')} disabled={serving} onClick={()=>{cancelGuide();if(unavailable){openBoard();return;}setShowDetails(false);guide('board',()=>controller.current?.walkToBoard?.());}} style={{'--place-color':'#c9b898'} as React.CSSProperties}><span className="town-dot"/>{tl('Board','Tablón')} {challengesDone}/3</button>
    </nav>
    {destinationScroll.left&&<button className="town-dest-scroll town-dest-left" aria-label={tl('Earlier destinations','Destinos anteriores')} onClick={()=>scrollDestinations(-1)}>‹</button>}
    {destinationScroll.right&&<button className="town-dest-scroll town-dest-right" aria-label={tl('More destinations','Más destinos')} onClick={()=>scrollDestinations(1)}>›</button>}
    </div>
    {room!=='city'&&<div className="town-room-bar">
    {room==='bank'&&<div className="town-interior-nav"><span>{tl(tl('Inside the bank','Dentro del banco'),'Dentro del banco')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToTeller?.();}}>{tl(tl('Walk to teller','Ir al cajero'),'Ir al cajero')}</button><button className="town-walk-exit" onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToExit?.();}}>{tl(tl('Walk to exit','Ir a la salida'),'Ir a la salida')}</button><button onClick={()=>{cancelGuide();controller.current?.leaveBank?.();}}><span className="town-exit-long">{tl(tl('Return to square ↗','Volver a la plaza ↗'),'Volver a la plaza ↗')}</span><span className="town-exit-short">{tl('Exit ↗','Salir ↗')}</span></button></div>}
    {room==='work'&&<div className="town-interior-nav"><span>{tl(tl('Your office','Tu oficina'),'Tu oficina')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToManager?.();}}>{tl(tl('Walk to your manager','Ir con tu jefe'),'Ir con tu jefe')}</button><button className="town-walk-exit" onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToExit?.();}}>{tl(tl('Walk to exit','Ir a la salida'),'Ir a la salida')}</button><button onClick={()=>{cancelGuide();controller.current?.leaveWork?.();}}><span className="town-exit-long">{tl(tl('Back to the square','Volver a la plaza'),'Volver a la plaza')}</span><span className="town-exit-short">{tl('Exit ↗','Salir ↗')}</span></button></div>}
    {room==='college'&&<div className="town-interior-nav"><span>{tl('Community College','Colegio Comunitario')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToRegistrar?.();}}>{tl('Walk to the registrar','Ir con la secretaria')}</button><button className="town-walk-exit" onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToExit?.();}}>{tl('Walk to exit','Ir a la salida')}</button><button onClick={()=>{cancelGuide();controller.current?.leaveCollege?.();}}><span className="town-exit-long">{tl('Back to the square','Volver a la plaza')}</span><span className="town-exit-short">{tl('Exit ↗','Salir ↗')}</span></button></div>}
    {room==='home'&&<div className="town-interior-nav"><span>{tl(tl('Your place','Tu casa'),'Tu casa')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToDesk?.();}}>{tl(tl('Walk to desk','Ir al escritorio'),'Ir al escritorio')}</button><button className="town-walk-exit" onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToExit?.();}}>{tl(tl('Walk to exit','Ir a la salida'),'Ir a la salida')}</button><button onClick={()=>{cancelGuide();controller.current?.leaveHome?.();}}><span className="town-exit-long">{tl(tl('Back to the square ↗','Volver a la plaza ↗'),'Volver a la plaza ↗')}</span><span className="town-exit-short">{tl('Exit ↗','Salir ↗')}</span></button></div>}
    {room==='property'&&<div className="town-interior-nav"><span>{tl(tl('Estate office','Inmobiliaria'),'Inmobiliaria')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToAgent?.();}}>{tl(tl('Walk to agent','Ir con el agente'),'Ir con el agente')}</button><button className="town-walk-exit" onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToExit?.();}}>{tl(tl('Walk to exit','Ir a la salida'),'Ir a la salida')}</button><button onClick={()=>{cancelGuide();controller.current?.leaveProperty?.();}}><span className="town-exit-long">{tl(tl('Return to square ↗','Volver a la plaza ↗'),'Volver a la plaza ↗')}</span><span className="town-exit-short">{tl('Exit ↗','Salir ↗')}</span></button></div>}
    {room==='exchange'&&<div className="town-interior-nav"><span>{tl(tl('Trading floor','Sala de operaciones'),'Sala de operaciones')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToBroker?.();}}>{tl(tl('Walk to broker','Ir con el corredor'),'Ir con el corredor')}</button><button className="town-walk-exit" onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToExit?.();}}>{tl(tl('Walk to exit','Ir a la salida'),'Ir a la salida')}</button><button onClick={()=>{cancelGuide();controller.current?.leaveExchange?.();}}><span className="town-exit-long">{tl(tl('Return to square ↗','Volver a la plaza ↗'),'Volver a la plaza ↗')}</span><span className="town-exit-short">{tl('Exit ↗','Salir ↗')}</span></button></div>}
    {room==='cafe'&&<div className="town-interior-nav"><span>{state.cafe?tl('Your café','Tu café'):tl('Café viewing','Visita al café')}</span><button onClick={()=>{cancelGuide();setShowDetails(false);controller.current?.walkToCafeCounter?.();}}>{tl(tl('Walk to counter','Ir al mostrador'),'Ir al mostrador')}</button><button onClick={()=>{setRoomBrief(false);setCafePlay(true);setJournal(false);setShowDetails(true);}}>{tl(tl('Play a shift','Jugar un turno'),'Jugar un turno')}</button><button onClick={()=>{setRoomBrief(false);setCafePlay(false);setJournal(false);setShowDetails(true);}}>{tl(tl('Manage café','Administrar el café'),'Administrar el café')}</button><button onClick={()=>{cancelGuide();if(unavailable){setRoom('city');setShowDetails(false);}else controller.current?.leaveCafe?.();}}><span className="town-exit-long">{tl(tl('Return to square ↗','Volver a la plaza ↗'),'Volver a la plaza ↗')}</span><span className="town-exit-short">{tl('Exit ↗','Salir ↗')}</span></button></div>}
    <button className="town-location-help" aria-label={`${tl('What can I do here?','¿Qué puedo hacer aquí?')} · ${tl('Actions, costs & rewards','Acciones, costos y recompensas')}`} onClick={()=>{setRoomBrief(true);setJournal(false);setShowDetails(true);}}><span className="town-help-text">{tl('What can I do here?','¿Qué puedo hacer aquí?')} · {tl('Actions, costs & rewards','Acciones, costos y recompensas')}</span><span className="town-help-icon" aria-hidden="true">?</span></button>
    </div>}
    ...
    <div className={`town-body${showDetails ? ' town-details-open' : ''}${serving?' town-serving-active':''}`}>
      <section className="town-viewport" aria-label={tl(tl('Neighbourhood','Vecindario'),'Vecindario')}>
        ...
        {unavailable ? <div className="town-fallback"><Building2 size={36} /><h3>{tl(tl('Explore with the destination buttons','Explora con los botones de destino'),'Explora con los botones de destino')}</h3><p>{tl(tl('This browser cannot display the 3D view. Your purchases and progress still work.','Este navegador no puede mostrar la vista 3D. Tus compras y tu progreso siguen funcionando.'),'Este navegador no puede mostrar la vista 3D. Tus compras y tu progreso siguen funcionando.')}</p></div> : <>
          ...
          {statsOn&&stats&&<div className="town-stats" aria-hidden="true">{stats.fps} fps · {stats.calls} calls · {Math.round(stats.triangles/1000)}k tris{qualityLevel?` · ${qualityLevel}`:''}</div>}
          {sleeping!==null&&<div className={`town-sleep-overlay${reduceMotion?' town-sleep-still':''}`} role="status"><span>🌙</span><strong>{tl('Good night','Buenas noches')}</strong><p>{tl('Month','Mes')} {sleeping} {tl('begins at sunrise','empieza al amanecer')} ☀️</p></div>}
          {celebrating&&<div className="town-earned" role="status"><span>✦</span><strong>{celebrating.title}</strong><p>{celebrating.text}</p></div>}
          ...
          <div className="town-joystick" role="group" aria-label={tl(tl('Movement joystick. Drag to walk, or use W A S D or arrow keys.','Joystick de movimiento. Arrastra para caminar o usa W A S D o las flechas.'),'Joystick de movimiento. Arrastra para caminar o usa W A S D o las flechas.')} tabIndex={0}
            onPointerDown={event=>{event.preventDefault();if(stickPointer.current!==null)return;cancelGuide();stickPointer.current=event.pointerId;event.currentTarget.setPointerCapture(event.pointerId);updateStick(event);}}
          ...
          <div className="town-camera-help"><span>{tl('Walk into doors to enter','Camina hacia la puerta para entrar')}</span><span>{tl(tl('Drag to look','Arrastra para mirar'),'Arrastra para mirar')}</span><span>{tl(tl('Tap to walk','Toca para caminar'),'Toca para caminar')}</span><span>{tl(tl('WASD · Shift to jog','WASD · Shift para trotar'),'WASD · Shift para trotar')}</span></div>
        </>}
      </section>
      <aside className="town-details" ref={details} tabIndex={-1} aria-label={tl(tl('Location opportunities','Oportunidades del lugar'),'Oportunidades del lugar')} style={{display:showDetails?'block':'none'}}>
        ...
        {room==='cafe'&&<CafeCinematic/>}
        {roomBrief&&location ? <TownLocationGuide location={location} onContinue={continueRoom} label={roomAction}/> : <>
        {location&&!journal&&<details className="town-purpose-disclosure"><summary>{tl('Actions, costs & rewards here','Acciones, costos y recompensas aquí')}</summary><TownLocationGuide location={location} compact/></details>}
        {garageOpen&&room==='city' ? <GaragePanel state={state} disabled={disabled} onBuyVehicle={onBuyVehicle} onSellVehicle={onSellVehicle}/> : board&&room==='city' ? <NoticeBoardPanel state={state} disabled={disabled} onNextMonth={onNextMonth} onClaimQuest={onClaimQuest} onOpenQuests={onOpenQuests}/> : rosa&&room==='city' ? <AdvisorPanel state={state} onGo={goTo}/> : room==='college' ? <CollegePanel state={state} disabled={disabled} onEnroll={onEnroll} onOpenLife={onOpenLife}/> : room==='work' ? <WorkPanel state={state} disabled={disabled} onPromote={onPromote} onOpenLife={onOpenLife} onAskRaise={onAskRaise} onSwitchCareer={onSwitchCareer} onJobSearch={onJobSearch} onAcceptPlan={onAcceptPlan} workActions={workActions} onMonthlyAction={onMonthlyAction}/> : room==='home' ? <HomePanel state={state} disabled={disabled} onSleep={onSleep?()=>{controller.current?.timeLapse?.();setSleeping(state.month+1);onSleep();}:undefined} onChangeLifestyle={onChangeLifestyle} onGo={goTo} onStartHustle={onStartHustle} onStopHustle={onStopHustle} onChooseUpgrade={onChooseUpgrade} workActions={workActions} onMonthlyAction={onMonthlyAction} onCollegeFund={onCollegeFund}/> : journal ? <>
        ...
        </> : room==='property'&&spot==='agent'?<PropertyPanel state={state} disabled={disabled} onBuy={(item,quantity)=>{pendingPurchase.current={itemId:item.id,quantity:quantityOf(item.id)};setLastPurchase(null);onBuy(item,quantity);}} onMortgage={onMortgage} onOpenMoney={()=>onOpenMoney('invest','property')}/> : room==='exchange'&&spot==='broker'?<ExchangePanel state={state} disabled={disabled} onBuy={(item,quantity)=>{pendingPurchase.current={itemId:item.id,quantity:quantityOf(item.id)};setLastPurchase(null);onBuy(item,quantity);}} onSell={onSell?id=>{setLastPurchase(null);pendingPurchase.current=null;onSell(id);}:undefined} onOpenMoney={()=>onOpenMoney('invest','exchange')}/> : room==='bank'&&spot==='teller'?<TellerPanel state={state} disabled={disabled} onTransfer={onTransfer} loans={loans} onLoans={()=>onOpenMoney('bank','bank')} onReserve={()=>onAction?.('reserve')} onFileClaim={onFileClaim} onBuyPolicy={onBuyPolicy} onCancelPolicy={onCancelPolicy} onBusiness={()=>{setShowDetails(false);if(cart)visitCart();else move('business');}}/> : place ? <>
