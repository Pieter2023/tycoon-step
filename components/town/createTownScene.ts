...
import { whenAudioReady } from '../../services/audioService';
...
export function createTownScene(host: HTMLDivElement, onNear: (id: TownPlaceId | null) => void, onInteract: () => void, onFailure: () => void, reducedMotion: boolean, onReady?: () => void, options:TownSceneOptions = {}): TownController {
  const atelier=options.art==='atelier', library=atelier?createAtelierMaterials():undefined;
  ...
  const office = createTownProperty(library); scene.add(office.root);
  ...
  let routeBlockedFor=0;
  let workApron:THREE.Group|undefined;
  ...
  const cart=createCoffeeCart(library);outdoors.add(cart.root);let cartLicensed=false,paused=false;
  ...
  const officeFacade=createHomeFacade({x:-WORK.x,z:WORK.z},OFFICE_PALETTE);officeFacade.root.scale.x=-1;outdoors.add(officeFacade.root);
  {const b=officeFacade.bounds;officeFacade.bounds=new THREE.Box3(new THREE.Vector3(-b.max.x,b.min.y,b.min.z),new THREE.Vector3(-b.min.x,b.max.y,b.max.z));
   const frame=new THREE.Mesh(new THREE.BoxGeometry(.35,2.6,1.7),new THREE.MeshStandardMaterial({color:'#eef1f3'}));frame.position.set(16.55,1.5,WORK.z);frame.castShadow=true;outdoors.add(frame);
   const door=new THREE.Mesh(new THREE.BoxGeometry(.08,2.1,1.0),new THREE.MeshStandardMaterial({color:'#3f5a73'}));door.position.set(16.36,1.27,WORK.z);outdoors.add(door);
   const knob=new THREE.Mesh(new THREE.SphereGeometry(.05,8,6),new THREE.MeshStandardMaterial({color:'#c9d2d9',metalness:.6,roughness:.3}));knob.position.set(16.3,1.2,WORK.z-.35);outdoors.add(knob);
   const step=new THREE.Mesh(new THREE.BoxGeometry(.6,.12,1.4),new THREE.MeshStandardMaterial({color:'#c3ccd3'}));step.position.set(16.1,.26,WORK.z);outdoors.add(step);
   const sign=document.createElement('canvas');sign.width=512;sign.height=128;const ink=sign.getContext?.('2d');if(ink){ink.fillStyle='#233b4d';ink.fillRect(0,0,512,128);ink.fillStyle='#f5f0e4';ink.font='600 56px sans-serif';ink.textAlign='center';ink.fillText('MAIN ST · OFFICES',256,84);}
   const texture=new THREE.CanvasTexture(sign);texture.colorSpace=THREE.SRGBColorSpace;const plate=new THREE.Mesh(new THREE.PlaneGeometry(1.5,.375),new THREE.MeshBasicMaterial({map:texture}));plate.position.set(16.33,2.95,WORK.z);plate.rotation.y=-Math.PI/2;outdoors.add(plate);}
  ...
  // Sleeping through to the next month plays the night as a time-lapse (timeLapse) and the day carries on from that morning (phaseShift).
  const LAPSE_SECONDS=2.8, MORNING=.03;
  let phaseShift=0, lapse:{start:number;from:number;to:number}|null=null;
  const naturalPhase=()=>((dayPhase(townMonth,elapsed,reducedMotion)+phaseShift)%1+1)%1;
  const currentPhase=()=>{
    if(phaseOverride!==undefined)return phaseOverride;
    if(lapse){const t=Math.min(1,(elapsed-lapse.start)/LAPSE_SECONDS),e=t*t*(3-2*t),p=lapse.from+(lapse.to-lapse.from)*e;
      if(t>=1){const to=lapse.to%1;lapse=null;phaseShift+=to-naturalPhase();return to;}
      return p%1;}
    return naturalPhase();
  };
  ...
  let soundEnabled=false,lastStep=0,cancelAudio:(()=>void)|undefined;
  const stepSound=(speed:number)=>{if(!soundEnabled||!ambience)return;ambience.step(inside,speed);};
  ...
  let playerAtelier=false;let updateExpression:((seconds:number,reducedMotion:boolean)=>void)|undefined;
  ...
  const blinks: ((seconds:number, reducedMotion:boolean, hold?:number)=>void)[] = [];
  let blinkHold: number | undefined;   // dev handle __town.blink(amount): hold every lid at that closure for stills
  ...
  if(library){
    load('/models/town/teller-desk.glb?v=civic1').then(g=>{if(alive)bank.installDesk(g.scene);}).catch(()=>{});
    load('/models/town/estate-desk.glb?v=civic1').then(g=>{if(alive)office.installDesk(g.scene);}).catch(()=>{});
  }
  ...
  const MODEL_VERSION = '20260926b', PEOPLE_VERSION = '20260925e', HERO_VERSION = '20260926a';
  ...
  const keyboard = (event: KeyboardEvent) => {
    if (event.ctrlKey || event.altKey || event.metaKey || paused || !!serviceStage || (event.target instanceof HTMLElement && (event.target.matches('input,textarea,select') || event.target.isContentEditable))) return;
    ...
    if (key === 'r') { yaw=atelier&&!inside?-.55:.12;const preset=cameraPreset(cameraMode,inside);pitch=preset.pitch;zoomDistance=preset.distance; }
  };
  ...
  const dayPhaseDark=()=>daylight(currentPhase(),rainy).lamps>.4;
  ...
  const step = (now:number, frameMs:number, dt:number) => {
    ...
    updateExpression?.(elapsed,reducedMotion);for(const blink of blinks)blink(elapsed,reducedMotion,blinkHold);
    ...
    if(ready && !paused) {
      ...
      if (!input.x && !input.z && path.length) {
        ...
        if(occupied&&Math.hypot(path[0].x-player.position.x,path[0].z-player.position.z)<.75)path=path.slice(1);
        const intent=followRoute(player.position,path,keys.has('shift')||!!serviceStage,(a,b)=>inside||segmentClear(a,b));
        path=intent.path;movement=intent.movement;speed=intent.speed;
      }
      if(playerAtelier)speed*=.84;
      if(movement.x||movement.z){
        const heading=Math.atan2(movement.x,movement.z);
        player.rotation.y=turnTowards(player.rotation.y,heading,dt,10);
        speed*=turnSpeedFactor(player.rotation.y,heading);
      }
      ...
      if(!inside){
        ...
        for(const pass of traffic?.update(dt,here,rainy||dayPhaseDark(),true)??[])ambience?.carPass(-pass.pan*Math.cos(yaw),pass.closeness);
        life.update(dt,elapsed,here,true);cyclist?.update(dt,elapsed);eventStage.update(elapsed);if(fireworks.update(dt,elapsed,celebrating||elapsed<momentUntil))ambience?.firework(elapsed<momentUntil?.8:.45);
      ...
      if(options.autoDoors && !inside && !serviceStage && (input.x || input.z)){
        const entry=doorwayEntry(player.position,movement,now,doorLockedUntil);
        if(entry)transition(true,entry);
      }
    ...
    const light=inside?daylight(.22,false):daylight(currentPhase(),rainy);
    ...
    const portrait=camera.aspect<.8 && (cafeInside || atelier || !!options.review);
    const framedDistance=zoomDistance*(portrait?Math.min(1.8,1.05/camera.aspect):1);
    ...
    // The college is a foreground cutaway when viewing the square from behind it.
    // Its front remains visible when approaching the entrance from the street.
    if(atelier)collegeFacade.root.visible=inside || !(desiredCamera.z>COLLEGE.z+.5 && Math.abs(desiredCamera.x-COLLEGE.x)<14);
    cameraDirection.copy(desiredCamera).sub(cameraTarget).normalize();cameraRay.set(cameraTarget,cameraDirection);
    let obstructed=false;
    if(!inside){
      for(const wall of walls)if((!atelier||collegeFacade.root.visible||wall!==collegeFacade.bounds)&&cameraRay.intersectBox(wall,hitPoint)){const length=cameraTarget.distanceTo(hitPoint)-.45;if(length<cameraTarget.distanceTo(desiredCamera)){obstructed=true;desiredCamera.copy(cameraTarget).addScaledVector(cameraDirection,Math.max(.7,length));}}
    ...
    if(serviceStage==='serve'&&!reducedMotion&&playerActor&&pedestrians[0]){
      ...
      const giver=playerActor.root.getObjectByName(playerAtelier?'EXPORT_Grip1':'Grip1'),receiver=pedestrians[0].root.getObjectByName('Grip1');
  ...
  return {
    ...
    enterWork(){if(spot==='work'&&!inside)transition(true,'work');},leaveWork(){transition(false);},
    ...
    // The soundscape plays through the game's one audio engine (services/audioService.ts) and starts on the player's first click if audio cannot start yet.
    setSound(enabled){soundEnabled=enabled;cancelAudio?.();cancelAudio=undefined;if(!enabled){ambience?.dispose();ambience=undefined;return;}if(ambience)return;cancelAudio=whenAudioReady(audio=>{cancelAudio=undefined;if(!alive||!soundEnabled||ambience)return;ambience=createTownAmbience(audio.ctx,audio.world,audio.reverb);ambience.update(rainy,inside,document.hidden);});},
