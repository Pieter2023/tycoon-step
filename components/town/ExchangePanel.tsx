...
export default function ExchangePanel({ state, disabled, onBuy, onSell, onOpenMoney }: Props) {
  ...
  return <>
    ...
    <div className="town-offers">
      {EXCHANGE_ITEMS.map(id => {
        ...
        return <article key={id} className="town-offer">
          ...
          {held && <p className="town-holding">{tl('You hold','Tienes')} {held.quantity} {tl('worth','por valor de')} {money(held.value * held.quantity)} · {tl('paid','pagaste')} {money(held.costBasis * held.quantity)} · {unrealised(held) >= 0 ? tl('up','de ganancia') : tl('down','de pérdida')} {money(Math.abs(unrealised(held)))} {tl('unrealised','sin realizar')}</p>}
    ...
    <div className="town-lesson"><strong>{tl('Time in the market','Tiempo en el mercado')}</strong>
      ...
      {!journey.completed && journey.step === 2 && <p>{tl('Your first units need','Tus primeras unidades necesitan')} {HOLD_MONTHS} {tl('months of holding before this arc finishes. Price gains become cash when you sell; dividends pay cash separately.','meses en cartera antes de que termine este recorrido. Las ganancias de precio se vuelven efectivo al vender; los dividendos pagan por separado.')}</p>}</div>
