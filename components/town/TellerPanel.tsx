...
export default function TellerPanel({state,disabled,onTransfer,loans,onLoans,onReserve,onBusiness,onFileClaim,onBuyPolicy,onCancelPolicy}:{state:GameState;disabled:boolean;onTransfer?:(transfer:BankTransfer)=>void;loans:TownLoan[];onLoans:()=>void;onReserve:()=>void;onBusiness:()=>void;onFileClaim?:()=>void;onBuyPolicy?:(id:PolicyId,deductible:number)=>void;onCancelPolicy?:(id:PolicyId)=>void}) {
  ...
  return <>
    ...
    {tab==='insurance'?<InsurancePanel state={state} disabled={disabled} onBuyPolicy={onBuyPolicy} onCancelPolicy={onCancelPolicy}/>:tab==='savings'?<>
      ...
      {loans.map(loan=>{const r=loan.rate/12;const payment=r?Math.round(loan.amount*r/(1-Math.pow(1+r,-loan.term))):loan.amount/loan.term;return <article className="town-offer" key={loan.id}><h4>{loan.name}</h4><p>{money(loan.amount)} · {(loan.rate*100).toFixed(1)}% {tl('APR','tasa anual')} · {loan.term} {tl('months','meses')}</p><p><strong>{money(payment)}/{tl('month','mes')}</strong> · {tl('approximately','aproximadamente')} {money(payment*loan.term-loan.amount)} {tl('total interest.','de interés total.')}</p><p>{tl('Monthly surplus after payment','Excedente mensual después de la cuota')}: <strong>{money(cashFlow.income-cashFlow.expenses-payment)}</strong></p></article>;})}
