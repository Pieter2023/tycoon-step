...
export default function WorkPanel({ state, disabled, onPromote, onOpenLife, onAskRaise, onSwitchCareer, onJobSearch, onAcceptPlan, workActions, onMonthlyAction }: Props) {
  ...
  return <>
    ...
    {unemployed && <section className="town-work-block" aria-label={tl('Between jobs','Sin empleo')}>
      ...
      <p className="town-small">{benefit?.active ? `${tl('Unemployment benefit','Prestación por desempleo')}: ${money(benefit.monthly)}/${tl('mo','mes')} · ${benefit.monthsLeft} ${tl('left','restantes')}${benefit.searchedThisMonth ? '' : ` · ${tl('apply this month or it pauses','postula este mes o se pausa')}`}.` : tl('Laid off? The Community Bank teller files unemployment insurance: half your pay, up to six months, while you apply each month.','¿Te despidieron? El cajero del Banco Comunitario tramita el seguro de desempleo: la mitad de tu sueldo, hasta seis meses, mientras postules cada mes.')}</p>
    </section>}
