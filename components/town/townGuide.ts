...
export type GuideTarget = 'agent' | 'teller' | 'business' | 'cart' | 'cafe' | 'broker' | 'manager' | 'desk' | 'rosa' | 'board' | 'registrar';
...
export function guideNextHop(target: GuideTarget, c: Pick<GuideContext, 'room' | 'near' | 'spot'>): 'enterProperty' | 'walkToAgent' | 'enterBank' | 'walkToTeller' | 'enterCafe' | 'enterExchange' | 'walkToBroker' | 'enterWork' | 'walkToManager' | 'enterHome' | 'walkToDesk' | 'enterCollege' | 'walkToRegistrar' | 'arrived' | null {
  const { room, near, spot } = c;
  if (target === 'agent') { if (room === 'city') return near === 'property' ? 'enterProperty' : null; if (room === 'property') return spot === 'agent' ? 'arrived' : 'walkToAgent'; return null; }
