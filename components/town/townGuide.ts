import type { townJourney } from '../../services/townJourney';
import type { TownPlaceId } from './townWorld';
import type { TownSpot } from './createTownScene';

export type GuideTarget = 'teller' | 'business' | 'cart' | 'cafe';
export type GuideContext = {
  journey: ReturnType<typeof townJourney>;
  room: 'city' | 'bank' | 'cafe';
...
export function guideLabel(c: GuideContext): string {
  ...
  if (journey.action === 'finish') return journal && showDetails ? 'Complete my opening journey ✦' : journey.button;
...
export function guideNextHop(target: GuideTarget, c: Pick<GuideContext, 'room' | 'near' | 'spot'>): 'enterBank' | 'walkToTeller' | 'enterCafe' | 'arrived' | null {
