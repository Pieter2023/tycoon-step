...
import { decorateCart, type AtelierMaterials } from './townAtelier';
import { RoundedBoxGeometry } from 'three/examples/jsm/geometries/RoundedBoxGeometry.js';
export function createCoffeeCart(library?:AtelierMaterials){
  ...
  const oldMachine=add(.52,.44,.36,.38,1.38,0,'#d9b769'),oldTray=add(.25,.13,.29,.38,1.2,-.24,'#263e45');
  ...
  const practical=library?new THREE.PointLight('#ffd59a',0,5,2):undefined;
  const bulb=library?new THREE.Mesh(new THREE.BoxGeometry(.9,.018,.08),new THREE.MeshStandardMaterial({color:'#ead5ab',emissive:'#ffd295',emissiveIntensity:0})):undefined;
  if(practical){practical.position.set(0,2.2,0);cover.add(practical);}if(bulb){bulb.position.set(0,2.25,0);cover.add(bulb);}
  if(library){sign.visible=false;back.visible=false;decorateCart(root,cover,library);}
  return {root,setLight(level:number){if(practical)practical.intensity=level*12;if(bulb)(bulb.material as THREE.MeshStandardMaterial).emissiveIntensity=level*1.5;},installMachine(machine:THREE.Object3D){oldMachine.visible=oldTray.visible=false;machine.position.set(.32,1.165,0);root.add(machine);machine.traverse(o=>{if(o instanceof THREE.Mesh)o.castShadow=o.receiveShadow=true;});},presentCup(position:THREE.Vector3|null){servedCup.visible=!!position;if(position)servedCup.position.copy(root.worldToLocal(position.clone()));},setState(owned:boolean,upgraded:boolean){root.visible=owned;cover.visible=upgraded;crates.visible=upgraded;}};
}
