...
export default function NoticeBoardPanel({ state, onNextMonth, disabled, onClaimQuest, onOpenQuests }: { state: GameState; onNextMonth: () => void; disabled: boolean; onClaimQuest?: (id: string) => void; onOpenQuests?: () => void }) {
  ...
  return <>
    ...
    <section className="town-work-block" aria-label={tl('Quest log','Registro de misiones')}>
      ...
      <p className="town-small">{tl('Longer goals from the dashboard, pinned here with live progress. Claim the listed rewards to change your saved-game cash or stats.','Metas más largas del panel, fijadas aquí con progreso en vivo. Reclama las recompensas indicadas para cambiar el dinero o las estadísticas de tu partida.')}</p>
