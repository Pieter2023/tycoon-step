    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="glass-panel p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-xs text-slate-400 bg-slate-950/40 px-3 py-1 rounded-full border border-slate-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {(Object.entries(LIFESTYLE_OPTS) as [Lifestyle, typeof LIFESTYLE_OPTS[Lifestyle]][]).map(([key, opt]) => (
          <motion.div key={key} whileHover={{ scale: 1.02 }}
            ...
            className={`p-5 rounded-xl cursor-pointer transition-all border ${gameState.lifestyle === key
              ? 'bg-emerald-600/20 border-emerald-500/80 shadow-[0_0_15px_rgba(16,185,129,0.2)]'
            ...
            <div className="flex justify-between mt-3 text-sm">
              ...
              <span className="text-white font-bold">{formatMoney(opt.cost)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">Happiness:</span>
              <span className={`font-bold ${opt.happiness >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {opt.happiness >= 0 ? '+' : ''}{opt.happiness}
              </span>
            </div>
            {gameState.lifestyle === key && (
              <div className="mt-3 text-center">
                <span className="px-3 py-1 bg-emerald-500/20 text-emerald-300 rounded-full text-xs font-bold uppercase tracking-wider border border-emerald-500/30">Current Lifestyle</span>
              </div>
            )}
          </motion.div>
        ))}
      ...
      <div className="mt-8 glass-panel p-5">
            ...
            <div key={stat.label} className="glass-tile p-3">
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 h-2 bg-slate-700/50 rounded-full overflow-hidden">
                  <div className={`h-full bg-${stat.color}-500 transition-all shadow-[0_0_8px_rgba(var(--${stat.color}-500-rgb),0.5)]`}
        ...
        <div className="mt-4 glass-tile p-4 border-l-4 border-l-purple-500">
          <p className="text-white text-sm font-bold mb-2">How this impacts you</p>
          <ul className="text-slate-400 text-xs space-y-1.5 list-disc pl-4">
            <li>
              Monthly Actions: <span className="text-white font-bold">{gameState.monthlyActionsMax}</span> (energy/health/stress thresholds apply)
            </li>
            <li>
