  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const retryableMessage = typeof detail === 'string' && /rate limit|timeout|temporarily unavailable/i.test(detail);
        if (attempt < retries && (isTimeoutHtml(detail) || retryableStatus || retryableMessage)) {
