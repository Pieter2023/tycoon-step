...
export function investorJourney(state: GameState) {
  ...
  else if (step === 2) { const left = Math.max(0, (bought ?? state.month) + HOLD_MONTHS - state.month); title = tl('Let it ride','Déjalo correr'); detail = `${tl('Hold for','Consérvalo')} ${left} ${tl(left === 1 ? 'more month.' : 'more months.', left === 1 ? 'mes más.' : 'meses más.')} ${tl('Watch the price move: price gains become cash when you sell. Dividends pay cash separately.','Observa el precio: las ganancias de precio se vuelven efectivo al vender. Los dividendos pagan efectivo por separado.')}`; button = tl('Preview next month','Vista previa del próximo mes'); action = 'review'; }
