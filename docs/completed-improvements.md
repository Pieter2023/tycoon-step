...
## The dashboard shell, wired through the translator — September 7, 2026 (build 39)

Pieter's pick: "Wire the dashboard shell through the translator, keep going." Until now the Spanish toggle only reached the strings the June work had keyed; the v2 shell itself (sidebar, page headers, the Play page's simple view, the Money/Career/Learn/Life page chrome, the quick-actions menu, the first-steps mission, the signals stack, the mobile shell) was hard-coded English, so a Spanish player saw a Spanish city inside an English frame.

**What was done.** Eighteen `components/v2/*.tsx` files plus the shell parts of `App.tsx` now read every visible string through `useI18n().t` with keys of the form `shell.<component>.<slug>` (342 keys, English in `en.json`, Spanish in `es.json`, placeholders such as `{year}`, `{cash}`, `{remaining}` checked to match). Most of the work was mechanical: `scripts/i18n-shell-transform.py` rewrites JSX text nodes, string attributes, object-literal `title:/body:/label:` values and ternary literals into `t()` calls and inserts the hook after each component's parameter list; the sentences it cannot see (template strings, `{x} of {y}` interpolations, `Year {year} • Month {month}`, `Next Month (N)`, the overflow menu in App.tsx) were keyed by hand. Module-level helpers that build copy (`navMetaFor`, `getHealthTone`, `getAdvisorRecommendation`, `filtersFor`, `assetTypeLabelsFor`, `getEconomyTone`) take the translator as a parameter via the new exported `Translate` type, so they stay pure. The first transformer pass mangled type declarations and translated all-caps identifiers (`RECOVER`, `OVERTIME`); the shipped version refuses anything with a newline, `;=()[]|`, or that does not start with a letter or emoji, and the identifiers were restored.

**Out of scope, on purpose.** The tab bodies (`components/tabs/*`: invest list, bank, portfolio, reports, side-hustle and lifestyle detail) and the monthly-action cards (`services/monthlyActions.ts`) still print English; they are gameplay content rather than shell chrome and are the next slice if Pieter wants the whole adult game in Spanish. The mode picker and character select were already keyed in June.

**Verified in Chrome (5188, `tycoon_locale=es`, fresh adult game as Alex Chen).** Sidebar (Jugar/Dinero/Carrera/Aprender/Vida with their sublines, "Espacio de trabajo", "Ciclo"), header ("Año 1 • Mes 1", "Siguiente mes", "Avance automático APAGADO"), Play page ("Primeros pasos · 1 de 3", the repair choice, "Efectivo / Patrimonio neto / Ingresos pasivos", "3 de 3 acciones restantes…", "Meta de libertad: … cubierto", "Abrir el panel completo"), Money ("Cuartel del capital", "3.8 meses", the four sub-tabs), Career ("Sueldo mensual", "Experiencia: 0 meses", "Nivel de disrupción: 0% • Riesgo", skills with "Mejor puntaje/Intentos"), Learn, Life ("0 activos", "Hijos: 0"), and the quick-actions menu ("Guardar / Cargar … Volver al menú"). Switched back to English and confirmed "Next Month", "Autoplay OFF", "Money / Career" render as before. Dev-only gotcha seen twice: editing the translation JSON under Vite HMR re-creates the i18n context and the error boundary shows "useI18n must be used within I18nProvider" with raw keys; a page reload clears it and production is unaffected.
