...
- Fresh game: on a cleared origin start a new game and enter the city before any door: only the square is visible (no interior furniture on the promenade); the guide chains bank → teller → reserve, then Main Street → cart → permit → shift → preview → badge.
- Spanish: set the game language to Español in Accessibility (or `tycoon_locale=es`), open the city: navigation, caption, strip, location cards, every panel, Rosa's bubble, the teller's lines and the canvas boards read in Spanish; the guide's reserve/permit buttons still disable when unaffordable.
- Return to the city: close the month from the notice board; after the month preview's Advance the square reopens at the new month (unless an event, the annual report or a win needs the dashboard first).
- Tour: on a save with both earlier badges, the strip reads NEIGHBOURHOOD TOUR; each guide tap walks and enters (office → manager, home → desk, Rosa, board) and the counter advances; Review & finish awards Settled in. The loading overlay shows a download percentage on a cold cache. The city card stays enabled after financial freedom.
- Money Quest square: start a kids game, tap "Visit your square": a small figure on the square with four stops; the lemonade stand starts jobs, the toy shop buys and shows the shelf, the goal jar sets a goal, Next week advances in place and a kids event closes the square.
- Unemployment insurance: after a layoff (not a career change) the teller offers the claim; filing shows the live status; a month with a job application pays the benefit, a month without one pauses it, returning to work closes it. The dashboard job-loss event's "file for unemployment" option files it too.
- Side-hustle desk: at home, the desk panel lists running hustles with estimates and Stop, a Sprint action, and a catalogue with Start buttons that explain what blocks them; starting one shows a work table with laptop and parcels in the flat, stopping it removes them.
- One-on-one: at the manager's desk "Ask how you are doing" opens the conversation with the projected grade; "Agree a 3-month plan" lists goals that then track live with checkmarks; the plan is judged when it ends and the result appears as an event.
- Reviews and layoffs: after any January the office shows Last review with a grade and factors and the annual report carries the line; Job security prints the annual layoff risk with multipliers; while between jobs the office shows the runway and a once-a-month application, and the job board still allows a change.
- Career: at the manager's desk the raise buttons show odds and factors; asking applies a raise or a 2–3% consolation and disables both buttons for six months; the job board lists the other paths and Apply needs a second confirm; after a change the pay stub shows a month between jobs and the ladder restarts. Desk actions mirror the dashboard's monthly actions.
- Work: the Work button walks to Main Street Offices at the east end; inside, walk to the manager and open the panel: the pay stub lines add up to gross, tax is taken off, the promotion button is disabled until eligible, Study options / Career overview leave the city on the right tab. The payroll board on the back wall matches the panel.
- Keyboard/screen reader: Tab reaches every HUD control; opening a side panel or the Camera menu moves focus into it and Escape closes it (a second Escape closes the city); the canvas accepts E/Enter to enter a place; with Large text / High contrast on in Accessibility, the city HUD scales and brightens.
- Crowd: stand on the promenade lane (`window.__town.walk(-2.5, 6.35)` in dev) and watch residents step to the far side and pause if you block them; cart customers and bench sitters are unaffected.
- Freedom Day: with a winning save (or `window.__town.celebrate(true)` in dev for the fireworks only) the caption reads FREEDOM DAY, fireworks burst over the square, Rosa's first line is the win line.
- Characters: women are shorter and slimmer than men and wear fitted pants in brighter colours; men keep beards/caps and broad jackets. Walkers keep right so opposite directions never overlap; the player cannot walk through people (eased around) or cars (blocked). Stand a resident on the player's route (the cart queue at (1.1, 9.5) is one): the player steps around them and arrives; nobody gets wedged.
- Graphics: Camera menu → Graphics row shows Auto/Detailed/Balanced/Smooth; Smooth removes shadows, Detailed restores them, the choice survives a reload; the menu scrolls on short viewports. Auto shows a status note when it changes tier.
- Home: the Home button walks to 12 Square St, which sits in a two-storey townhouse (windows glow at night; the follow camera does not jam at the door or inside tree canopies); entering shows a flat furnished for the current lifestyle; the desk panel lists bills, mail and a lifestyle chooser that opens the confirm dialog and re-furnishes the flat. Rosa: the Rosa button walks to the west bench; her bubble shows her top line; the panel's "Show me" walks to the right place. The dashboard city card shows the guided step and Rosa's line.
...
- Family: with a spouse and children in the save, the flat shows the spouse by the kitchenette, a crib for a baby, a toy box and a scaled figure per child; the desk's Family block matches the monthly children expense, and "Put aside $250" moves cash to savings, updates the fund line and lands in the mail. Negative money anywhere in the city reads "-$1,471", never "$-1,471".
- Insurance: teller → Insurance tab lists three policies with the worst covered bill priced with and without cover; Take out cover adds the premium to the bills (turn preview "Insurance" line, fridge "Everything else"), posts the policy to the mail and marks the tab 🛡️. A covered event's uninsured option is reimbursed on the spot (claim in the mail); an option that already says "insurance" is not paid twice.
- Quest log: the Board panel lists rewards ready (Claim pays the same reward as the dashboard and updates the count), active quests with progress bars in their own units, and Up next; the paper on the 3D board shows the same challenges and quests with ticks.
- Vehicles: the owned car stands at the kerb by the townhouse; Garage walks to the bay; the panel's true monthly cost equals upkeep + lost value + interest; Sell pays the resale and clears the loan; Finance adds a CAR_LOAN whose payment shows in the bills; a negative net worth never prints a percentage above 100.
- Balance: `npx vitest run test/BalancePass.test.ts` after any change to premiums, event costs, car prices or college payback. On a medical event without health cover the "Use insurance" option is greyed with the lock note and the uninsured option is open; with cover both are open.
