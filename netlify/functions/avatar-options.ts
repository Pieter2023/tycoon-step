...
export const handler = async (event: { httpMethod?: string; body?: string }) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('avatar-options: upstream error', {
    ...
    console.log('avatar-options: success', {
      status: response.status,
      durationMs: Date.now() - startedAt,
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      console.error('avatar-options: timeout', {
        durationMs: Date.now() - startedAt
      });
      return {
        statusCode: 504,
    ...
    console.error('avatar-options: exception', {
      durationMs: Date.now() - startedAt,
